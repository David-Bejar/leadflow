        ...
        <div style={{ padding: '24px 20px', borderBottom: '1px solid #F1F5F9' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            ...
            <div>
              ...
              <span style={{ display: 'block', fontSize: 11, color: '#94A3B8', fontWeight: 500 }}>Administración</span>
            </div>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', minWidth: 0 }}>
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #F1F5F9', padding: '16px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          <button onClick={loadStats}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, border: '1px solid #E2E8F0', background: '#fff', fontSize: 13, fontWeight: 600, color: '#64748B', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '24px 28px' }}>
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
              {[1,2,3,4].map(i => <div key={i} style={{ height: 120, background: '#fff', borderRadius: 14, border: '1px solid #F1F5F9' }} />)}
            </div>
          ) : !stats ? null : (
            <>
              {/* OVERVIEW */}
              {section === 'overview' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[
                      { label: 'Total leads', val: stats.totals.leads, sub: 'formularios recibidos', color: '#6366F1', bg: '#EEF2FF', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
                      { label: 'Asignaciones', val: stats.totals.gestionados, sub: 'lead-empresa activas', color: '#0EA5E9', bg: '#F0F9FF', icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4' },
                      { label: 'Tasa conversión', val: `${stats.totals.conversion}%`, sub: 'sobre asignaciones', color: '#10B981', bg: '#ECFDF5', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
                      { label: 'Empresas activas', val: stats.totals.activos, sub: `de ${stats.totals.despachos} registradas`, color: '#8B5CF6', bg: '#F5F3FF', icon: 'M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z' },
                    ].map(s => (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <div style={{ background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, padding: '20px' }}>
                      ...
                      <p style={{ fontSize: 12, color: '#94A3B8', marginBottom: 20 }}>Nuevos formularios recibidos</p>
                    ...
                    <div style={{ background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, padding: '20px' }}>
                      ...
                      <p style={{ fontSize: 12, color: '#94A3B8', marginBottom: 20 }}>Distribución de leads recibidos</p>
                  ...
                  <div style={{ background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, padding: '20px' }}>
                    ...
                    <p style={{ fontSize: 12, color: '#94A3B8', marginBottom: 20 }}>Estado actual de todas las asignaciones lead-empresa</p>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid #F1F5F9' }}>
                            {['ID', 'Nombre', 'Categoría', 'Localización', 'Empresas asignadas', 'Fecha', ''].map(h => (
                        ...
                        <tbody>
                          {filteredLeads.map(lead => (
                            <tr key={lead.id} onClick={() => setSelectedLead(selectedLead?.id === lead.id ? null : lead)}
                              ...
                              <td style={{ padding: '12px 16px', color: '#64748B' }}>{lead.assigned_despachos?.length ?? 0} empresas</td>
                    ...
                    <div style={{ width: 340, background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, padding: '20px', flexShrink: 0, height: 'fit-content', position: 'sticky', top: 80 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                        {[
                          ...
                          ['Empresas asignadas', `${selectedLead.assigned_despachos?.length ?? 0}`],
                        ].map(([label, value]) => (
              ...
              {section === 'empresas' && (
                <div style={{ background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, overflow: 'hidden' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #F1F5F9' }}>
                        {['Empresa', 'Especialidades', 'Cola leads', 'Límite', 'Cerrados', 'Estado', 'Acciones'].map(h => (
                        ...
                        <tr key={d.id} style={{ borderBottom: '1px solid #F8FAFC' }}>
                          ...
                          <td style={{ padding: '14px 16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                              ...
                              <span style={{ fontSize: 12, fontWeight: 600, color: d.activos >= (d.max_leads_activos || 5) ? '#EF4444' : '#64748B' }}>
                                {d.activos}/{d.max_leads_activos || 5}
                              </span>
                            </div>
                          </td>
                          <td style={{ padding: '14px 16px' }}>
                            {editingLimite === d.id ? (
                              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                <input type="number" min={1} max={50} value={nuevoLimite}
                                ...
                                <button onClick={() => updateLimite(d.id, nuevoLimite)}
                                  style={{ padding: '4px 10px', background: '#6366F1', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>✓</button>
                                <button onClick={() => setEditingLimite(null)}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div style={{ background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, overflow: 'hidden' }}>
                    <div style={{ padding: '20px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0F172A', margin: 0 }}>Suscripciones y planes</h3>
                        <p style={{ fontSize: 13, color: '#94A3B8', margin: '4px 0 0' }}>Gestión de planes por empresa · Integración Stripe próximamente</p>
                      </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid #F1F5F9' }}>
                          {['Empresa', 'Plan actual', 'Límite leads', 'Leads activos', 'Cerrados', 'Estado pago'].map(h => (
                            <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                        ...
                        {stats.despachos.map((d) => {
                          ...
                          return (
                            <tr key={d.id} style={{ borderBottom: '1px solid #F8FAFC' }}>
                              ...
                              <td style={{ padding: '14px 16px' }}>
                                <span style={{ padding: '3px 10px', borderRadius: 20, background: planColors[planIdx] + '15', color: planColors[planIdx], fontSize: 11, fontWeight: 600 }}>
                                  {plans[planIdx]}
                                </span>
                              </td>
                              <td style={{ padding: '14px 16px', fontWeight: 600, color: '#374151' }}>{d.max_leads_activos || 5} leads</td>
                              <td style={{ padding: '14px 16px' }}>
                                <span style={{ fontWeight: 600, color: d.activos >= (d.max_leads_activos || 5) ? '#EF4444' : '#374151' }}>
                                  {d.activos}/{d.max_leads_activos || 5}
                                </span>
                              </td>
                              <td style={{ padding: '14px 16px', fontWeight: 600, color: '#059669' }}>{d.cerrados}</td>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
                    {[
                      { label: 'MRR estimado', val: `${stats.despachos.filter(d => d.active).reduce((acc, d) => acc + (d.max_leads_activos <= 3 ? 49 : d.max_leads_activos <= 5 ? 99 : 199), 0)}€`, sub: 'ingresos mensuales estimados', color: '#10B981', bg: '#ECFDF5' },
                      { label: 'Total asignaciones', val: stats.totals.gestionados, sub: 'lead-empresa activas', color: '#6366F1', bg: '#EEF2FF' },
                      { label: 'Empresas activas', val: stats.totals.activos, sub: 'con suscripción vigente', color: '#8B5CF6', bg: '#F5F3FF' },
                    ].map(s => (
                      <div key={s.label} style={{ background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, padding: '20px' }}>
